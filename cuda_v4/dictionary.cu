...
__global__ void mutate_and_check(uint *dict, unsigned int numwords, int* found) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        s_data[s_index] = dict[index]; // each thread load 16 times
    }
    __syncthreads();

    if(threadIdx.x >= numwords) return;
    ...
    int mutation_method = (int) blockIdx.x; // mutation_method is given by blockid
    mutation_method -= 1;
