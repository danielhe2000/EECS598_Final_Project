...
void get_mutate(password *target, password *dict, unsigned int numwords){
    int index = rand()%numwords;
    int z = (int)(rand()%253) - 1;
    memcpy(target, &(dict[index]), sizeof(password));
    
    if(z != -1){
        if (z==0){
            /* First letter uppercase */
            if (target->word[0] >= 'a' && target->word[0] <= 'z')
                target->word[0] +='A'-'a';
        }
        ...
        if (z==1){
            ...
            if (target->word[len-1] >= 'a' && target->word[len-1] <= 'z')
                target->word[len-1] += 'A'-'a';
        }
        ...
        if (z>=12 && z<=111){
            ...
            // iterator: z-12
            size_t len = target->length;
            target->word[len] = '0' + ((z-12)/10)%10;
            target->word[len+1] = '0' + (z-12)%10;
            target->length += 2;
        }
        ...
        if (z>=112 && z<=231){
            ...
            // iterator: z + (1900-112)
            size_t len = target->length;
            target->word[len] = '0' + ((z+1900-112)/1000)%10;
            target->word[len+1] = '0' + ((z+1900-112)/100)%10;
            target->word[len+2] = '0' + ((z+1900-112)/10)%10;
            target->word[len+3] = '0' + (z+1900-112)%10;
            target->length += 4;
        }
        ...
        if (z>=232 && z<=251){
            ...
            size_t len = target->length;
            memcpy(&(target->word[len]),seq[z-232].word,seq[z-232].length);
            target->length = len + seq[z-232].length;
        }
